...
export async function sendWelcomeEmail({ to, name }: WelcomeEmailProps) {
  ...
  <div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 16px; color: #333;">
    <h1 style="color: #2962FF;">Welcome to TradeSim, ${name}!</h1>
    ...
    <a href="https://tradesim.live/login" style="background-color: #2962FF; color: white; padding: 14px 25px; text-align: center; text-decoration: none; display: inline-block; border-radius: 8px; font-weight: bold;">
      Go to Your Dashboard
    </a>
    <p style="margin-top: 24px;">If you have any questions, feel free to contact our support team at any time.</p>
    <p>Happy trading!</p>
    <p><strong>The TradeSim Team</strong></p>
  </div>`;

  const mailOptions = {
    from: `"TradeSim" <${smtpConfig.auth.user}>`,
    to: to,
    subject: 'Welcome to TradeSim - Your Trading Journey Begins!',
    html: emailHtml,
  };
